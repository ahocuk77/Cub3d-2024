...
/*   Updated: 2024/03/26 18:44:34 by ahocuk           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_newline(t_game *game, int *x, int y)
{
	...
	{
		*x = tmp;
		while (*x < game->height && game->map.map[*x][y] != '1'
			&& game->map.map[*x][y] != '0')
			(*x)++;
	}
	else
...
int	vertical_validate(t_game *game, int x, int y)
{
	while (x < game->height && game->map.map[x][y] != ' ')
	{
		...
		if (x < game->height && game->map.map[x][y] == ' ')
		{
			...
			{
				while (x < game->height - 1 && game->map.map[x][y] == ' ')
					x = len_checker(game, x + 1, y);
				if (x < game->height - 1 && game->map.map[x][y] != '1')
					return (-1);
			}
		}
	}
